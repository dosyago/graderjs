...
System.register("service", ["fs", "path", "express", "chrome-launcher", "http-terminator", "config", "lib/common", "lib/protocol"], function (exports_3, context_3) {
    "use strict";
    var fs_1, path_2, express_1, chrome_launcher_1, http_terminator_1, config_js_2, common_js_1, protocol_js_1, PORT_DEBUG, MAX_RETRY, SITE_PATH, newSessionId, SessionId, retryCount;
    ...
    async function go() {
        ...
        **/
        if (common_js_1.DEBUG || process.argv[1].includes(`service_${config_js_2.default.name}`)) { // our startup cue
    ...
    async function run(app) {
        ...
        try {
            ({ service, port: ServicePort } = await start({ app, desiredPort: config_js_2.default.desiredPort }));
        }
        catch (e) {
            console.error(e);
        ...
        try {
            const expiredSessions = JSON.parse(fs_1.default.readFileSync(common_js_1.expiredSessionFile()).toString());
            expiredSessions.forEach(sessionId => {
                try {
                    fs_1.default.rmdirSync(common_js_1.sessionDir(sessionId), { recursive: true, maxRetries: 3, retryDelay: 700 });
                }
                catch (e) {
        ...
        fs_1.default.writeFileSync(common_js_1.expiredSessionFile(), JSON.stringify(undeletedOldSessions));
        // launch UI
    ...
    async function newBrowser({ ServicePort, sessionId, path: path = '/' }) {
        ...
        if (!fs_1.default.existsSync(common_js_1.temp_browser_cache(sessionId))) {
            console.log(`Temp browser cache directory does not exist. Creating...`);
            fs_1.default.mkdirSync(common_js_1.temp_browser_cache(sessionId), { recursive: true });
            console.log(`Created.`);
        }
        if (!fs_1.default.existsSync(common_js_1.app_data_dir(sessionId))) {
            console.log(`App data dir does not exist. Creating...`);
            fs_1.default.mkdirSync(common_js_1.app_data_dir(sessionId), { recursive: true });
            console.log(`Created.`);
        }
        ...
        const CHROME_OPTS = !common_js_1.NO_SANDBOX ? [
            ...
            `--disk-cache-dir=${common_js_1.temp_browser_cache(sessionId)}`,
        ...
        const LAUNCH_OPTS = {
            ...
            userDataDir: common_js_1.app_data_dir(sessionId),
        ...
        UI.shutdown = async () => {
            ...
            if (fs_1.default.existsSync(common_js_1.sessionDir(sessionId))) {
                try {
                    let expiredSessions = [];
                    try {
                        expiredSessions = JSON.parse(fs_1.default.readFileSync(common_js_1.expiredSessionFile()).toString());
                    }
                    catch (e) {
                        common_js_1.DEBUG && console.info(`Unable to read expired sessions file...`, e);
                    }
                    expiredSessions.push(sessionId);
                    fs_1.default.writeFileSync(common_js_1.expiredSessionFile(), JSON.stringify(expiredSessions));
                }
                catch (e) {
    ...
    function installCleanupHandlers({ ui, bg }) {
        ...
        const killService = async () => {
            try {
                ...
                common_js_1.DEBUG && console.info(`Error shutting down the browser...`, e);
            }
            if (bg.listening) {
                await stop(bg);
            }
    ...
    return {
        ...
        execute: function () {
            ...
            PORT_DEBUG = false;
            MAX_RETRY = 10;
            SITE_PATH = path_2.default.resolve(__dirname, 'public');
...
System.register("index", ["fs", "service", "lib/common"], function (exports_4, context_4) {
    "use strict";
    var fs_2, Service, Common, API, App;
...
System.register("app", ["index"], function (exports_5, context_5) {
    ...
    async function start() {
        ...
        const { UI: UI2, browser } = await index_js_1.default.ui.open();
        ...
        **/
        ...
        await index_js_1.default.ui.minimize();
        await index_js_1.default.util.sleep(2000);
